...
export default function TenantsPage() {
    ...
    if (values.id_type === 'passport_number' && values.passport_number) {
      ...
      if (!passportRegex.test(passport)) {
        setError('passport_number', {
          type: 'manual',
          message:
            'Passport must be 6-9 uppercase alphanumeric characters (A-Z, 0-9), no spaces or special characters.',
        });
        return;
      }
    ...
    if (values.id_type === 'national_id' && values.national_id) {
      ...
      if (
        ...
        setError('national_id', {
          type: 'manual',
          message:
            'National ID must be 6-18 uppercase alphanumeric characters (A-Z, 0-9), spaces and hyphens allowed.',
        });
        return;
      }
    ...
    try {
      ...
      setTenants((prev) => [
        ...prev,
        {
          ...
          tenancy_start_date: values.tenancy_start_date
            ? values.tenancy_start_date.toISOString()
            : '',
  ...
  const handleEditTenant = (tenant: Tenant) => {
    // logic for editing
    console.log('edit unit', tenant);
  };

  const handleDeleteTenant = (tenant: Tenant) => {
    // logic for deleting
    console.log('delete unit', tenant);
  };
